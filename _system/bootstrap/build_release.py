...
def main() -> None:
    ...
    metadata = json.loads(Path("_system/bootstrap/release.json").read_text(encoding="utf-8"))
    commit = subprocess.check_output(["git", "rev-parse", "HEAD"], text=True).strip()
    if len(commit) != 40 or any(character not in "0123456789abcdef" for character in commit):
        raise SystemExit("source commit must be a full SHA")

    basename = f"context-vault-{metadata['version']}"
    ...
    (args.output_dir / f"{basename}.release.json").write_text(
        json.dumps(
            {
                "schema_version": 1,
                "repository": "ContextNine/the-context-vault-template",
