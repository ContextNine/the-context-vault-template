#!/usr/bin/env python3
"""Fetch and safely fast-forward the vault before generated files change."""
...
def run_skill_sync(root: Path) -> None:
    script = root / "_system/agents/internal/src/sync_skills.py"
...
def preflight(root: Path, remote: str, branch: str, *, sync_skills: bool = True) -> str:
    ...
    if local != upstream:
        ...
        if local_is_base:
            result = git(root, "merge", "--ff-only", remote_ref, check=False)
            if result.returncode != 0:
                detail = result.stderr.strip() or result.stdout.strip() or "local changes prevent the update"
                raise PreflightError(f"safe fast-forward blocked; working tree preserved: {detail}")
            state = "updated"
