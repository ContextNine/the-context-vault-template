...
BEGIN_SHARED_VIEWS = f"  # BEGIN {MARKER}: epic views"
END_SHARED_VIEWS = f"  # END {MARKER}: epic views"
...
def vault_view_content(epics: list[Epic]) -> str:
    ...
    lines.append(END_SHARED_VIEWS)
    return "\n".join(lines) + "\n"


def update_system_kanban_views(root: Path, context_roots: list[Path], dry_run: bool) -> str:
    path = root / "_system/_obsidian/bases/tasks-kanban-v1.base"
    if not path.exists():
        return "missing"

    epics = [epic for context_root in context_roots for epic in discover_epics(context_root)]
    block = vault_view_content(epics)
    text = path.read_text(encoding="utf-8", errors="replace")

    start = text.find(BEGIN_SHARED_VIEWS)
    end = text.find(END_SHARED_VIEWS)
    if start != -1 and end != -1 and end > start:
        end += len(END_SHARED_VIEWS)
...
def sync(root: Path, teamspace_folders: str | None, include_archived: bool, dry_run: bool) -> int:
    ...
    result = update_system_kanban_views(root, context_roots, dry_run)
    print(f"shared tasks-kanban-v1 epic views: {result}: _system/_obsidian/bases/tasks-kanban-v1.base")
    return 0
