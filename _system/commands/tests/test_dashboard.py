...
class DashboardChecklistTests(unittest.TestCase):
    ...
    def test_dashboard_uses_quick_links_layout(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
        ...
        self.assertIn("### Checklist for Wed, 23rd July\n", rendered)
        self.assertIn("##### Quick Links\n", rendered)
        self.assertIn("#### Periodic Note Sources\n", rendered)
    ...
    def test_quick_links_include_periodic_rollups_and_conditional_month(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            without_month = dashboard.dashboard_periodic_link_lines(root, periods, day)
            monthly_rollup = root / "_system/_obsidian/periodic/monthly/2025-07.md"
            monthly_rollup.parent.mkdir(parents=True)
            monthly_rollup.write_text("monthly\n", encoding="utf-8")
            with_month = dashboard.dashboard_periodic_link_lines(root, periods, day)

        joined = "\n".join(without_month)
        self.assertIn("[[_system/_obsidian/periodic/daily/2025-07-23|Daily Note - 07-23]]", joined)
        self.assertIn("[[_system/_obsidian/periodic/weekly/2025-W30|Weekly Note - 2025-W30]]", joined)
        self.assertIn("[[_system/_obsidian/periodic/quarterly/2025-Q3|Quarterly Note - 2025-Q3]]", joined)
        self.assertIn("[[_system/_obsidian/periodic/yearly/2025|Yearly Note - 2025]]", joined)
        self.assertNotIn("Monthly Note", joined)
        self.assertIn(
            "[[_system/_obsidian/periodic/monthly/2025-07|Monthly Note - July 2025]]",
            "\n".join(with_month),
        )
