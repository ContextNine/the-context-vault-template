...
""",
    ...
    def test_carries_from_latest_daily_note_when_calendar_days_are_missing(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            entity = make_periodic_entity(
                root,
...
""",
            ...
            previous = entity / "_obsidian/periodic/daily/2026-06-09.md"
            previous.parent.mkdir(parents=True)
            previous.write_text(
                """---
type: periodic
...
""",
            ...
            current = entity / "_obsidian/periodic/daily/2026-06-12.md"
            self.assertIn("- [ ] survives a two-day gap", current.read_text(encoding="utf-8"))
