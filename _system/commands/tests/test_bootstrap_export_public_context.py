...
class PublicContextExportTests(unittest.TestCase):
    ...
    def test_system_root_markdown_is_private_by_default(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (system / "README.md").write_text("public system readme\n", encoding="utf-8")
            (system / "PRIVATE-NOTE.md").write_text("private note\n", encoding="utf-8")
            (system / "NEW-NOTE.md").write_text("private by default\n", encoding="utf-8")
            config = {
                "export_root": str(export_root),
                "copy_obsidian": "exact",
                "system_root_markdown_allow_paths": ["_system/README.md"],
            }
            exporter = BootstrapExporter(
                root=root,
                config=config,
            ...
            self.assertTrue((export_root / "_system/README.md").exists())
...
"""
    ...
    def test_public_export_regenerates_system_bases_for_target_contexts(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp) / "source"
            export_root = Path(tmp) / "public"
            source_brand = "matt" + "-derman"
            ...
            target_bootstrap = root / "_system/bootstrap/bootstrap_vault.py"
            target_bootstrap.parent.mkdir(parents=True)
            shutil.copy2(source_bootstrap, target_bootstrap)

            bases = root / "_system/_obsidian/bases"
...
""",
            ...
            kanban_v1 = (export_root / "_system/_obsidian/bases/tasks-kanban-v1.base").read_text(encoding="utf-8")
            ...
            content_calendar = (export_root / "_system/_obsidian/bases/content-calendar.base").read_text(encoding="utf-8")
