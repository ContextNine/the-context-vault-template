...
def dashboard_periodic_link_lines(root: Path, periods: dict[str, str], today: dt.date) -> list[str]:
    links = [
        obsidian_link(
            f"_system/_obsidian/periodic/daily/{periods['daily']}.md",
            f"Daily Note - {today.strftime('%m-%d')}",
        ),
        obsidian_link(
            f"_system/_obsidian/periodic/weekly/{periods['weekly']}.md",
            f"Weekly Note - {periods['weekly']}",
        ),
    ]
    monthly_path = root / VAULT_PERIODIC_DIR / "monthly" / f"{periods['monthly']}.md"
    if monthly_path.exists():
        links.append(
            obsidian_link(
                f"_system/_obsidian/periodic/monthly/{periods['monthly']}.md",
                f"Monthly Note - {month_label(periods['monthly'])}",
            )
        )
    links.extend(
        [
            obsidian_link(
                f"_system/_obsidian/periodic/quarterly/{periods['quarterly']}.md",
                f"Quarterly Note - {periods['quarterly']}",
            ),
            obsidian_link(
                f"_system/_obsidian/periodic/yearly/{periods['yearly']}.md",
                f"Yearly Note - {periods['yearly']}",
            ),
        ]
    )
    return links
...
def dashboard_markdown(
    ...
    periodic_link_lines = dashboard_periodic_link_lines(root, periods, today)
    return f"""---
type: dashboard
generated: true
