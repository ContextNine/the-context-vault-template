...
def clean_system_inbox_name(source: Path, note: Path | None) -> str:
    stem, suffix = os.path.splitext(clean_name(source.name))
    match = GENERIC_PASTE_RE.match(stem)
    if not match:
        return stem + suffix
    prefix = slugify(note.stem if note else "system-inbox")
...
def target_relative_for_source(source: Path, note_root: str, reason: str, note: Path | None = None) -> tuple[str, ...]:
    ...
    if is_under(source, SYSTEM_INBOX):
        return (clean_system_inbox_name(source, note),)

    if is_under(source, SYSTEM_ATTACHMENTS):
        source_rel = source.relative_to(SYSTEM_ATTACHMENTS)
        return clean_parts(source_rel.parts)
...
def should_migrate_reference(note: Path, source: Path) -> tuple[bool, str, bool]:
    ...
    if is_under(source, SYSTEM_INBOX):
        return True, "system-inbox-reference", True

    if is_under(source, SYSTEM_ATTACHMENTS) and "_inbox" not in source.relative_to(SYSTEM_ATTACHMENTS).parts:
        if note_root != "_system":
            return True, "_system-attachment-cross-root-reference", False
        return False, "", False
...
def cleanup_sources(
    ...
    cleanup_folders = IMPORT_FOLDERS + [SYSTEM_INBOX]
...
def verify() -> int:
    ...
    for note in iter_markdown_files():
        ...
        for span in iter_link_spans(text):
            ...
            if note_root != "_system" and is_under(source, SYSTEM_ATTACHMENTS):
                failures.append(f"{rel(note)} links to _system attachment {rel(source)}")
