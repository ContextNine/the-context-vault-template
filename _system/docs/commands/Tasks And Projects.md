...
TaskNotes `scheduled` and `due` fields are not exported to Google Calendar automatically. Use `$gws-i-custom-calendar` only when an explicit calendar event or time block is wanted.
