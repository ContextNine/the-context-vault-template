...
5. Stay with the user while the teamspace-folder wizard asks its questions. If the optional CTX9 skill system is selected, let its setup wizard finish too.
6. Verify the target contains `_system/bootstrap/release.json`, the selected teamspace folders, `.obsidian/plugins/context-nine/main.js`, an active `context-nine` entry in `.obsidian/community-plugins.json`, and a working `vault inventory` command.
