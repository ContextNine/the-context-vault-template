...
The root `AGENTS.md` is the concise entry point for this Obsidian Vault template. This document owns the detailed Git, host, routing, folder, and operating procedures. `CLAUDE.md` is a symlink to `AGENTS.md`; bootstrap/export copies the root instructions and agent setup scripts only ensure symlinks.

## Always

- When changing workflows, scripts, or anything under `_system/`, update the relevant skills and docs so the root `AGENTS.md`, this document, or skill discovery leads future agents to the operating contract.
